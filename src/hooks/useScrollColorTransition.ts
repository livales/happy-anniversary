...
export const useScrollColorTransition = () => {
  const [currentBgColor, setCurrentBgColor] = useState('linear-gradient(135deg, #FBF6ED 0%, #FDEEF1 50%, #F8E8EC 100%)');
  ...
  useEffect(() => {
    ...
    sections.forEach((section, index) => {
      ScrollTrigger.create({
        ...
        onEnter: () => {
          setCurrentBgColor(colorStops[index] || colorStops[colorStops.length - 1]);
        },
        onEnterBack: () => {
